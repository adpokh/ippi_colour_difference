...
class ColourDataset(Dataset):
    def __init__(self, features, labels, compress_range=(0.7, 1.0), expand_range=(1.0, 1.3)):
    ...
    def __getitem__(self, idx):
        ...
        L1, a1, b1, L2, a2, b2 = feature
        if label == 0:
            alpha = random.uniform(*self.compress_range)
            L2 = L1 + alpha * (L2 - L1)
            a2 = a1 + alpha * (a2 - a1)
            b2 = b1 + alpha * (b2 - b1)
        else:
            alpha = random.uniform(*self.expand_range)
            L2 = L1 + alpha * (L2 - L1)
            a2 = a1 + alpha * (a2 - a1)
            b2 = b1 + alpha * (b2 - b1)
        feature[3] = L2
...
train_dataset = ColourDataset(X_train, y_train)
test_dataset = ColourDataset(X_test, y_test)
...
# Сохраняем модель
model_path = "colour_net_model_lab_augm_2.pth"
torch.save({
